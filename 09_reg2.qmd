...
We can fit logistic regression models in R using `glm()` - for an example, we turn to `PSM0100` (`r attributes(pend10_lab$PSM0100)$label` yes = 2, no = 1) :
...
*There is a statistically significant relationship at the 0.001 level between age and the likelihood of using social media. With each additional year of age, there is a decrease of `r sprintf("%2.6f",abs(m2$coefficients[2]))` in the *logits* of the respondents using social media.*
...
# family = gaussian() provides a linear regression model, like lm() - but weighted
