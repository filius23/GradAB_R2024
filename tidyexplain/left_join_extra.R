...
lj_extra <- anim_df %>%
  arrange(obj, frame) %>%
  plot_data("left_join(dat1, dat2, by = 'id')") %>%
  animate_plot()

lj_extra <- animate(lj_extra)
anim_save(here::here("pic", "left-join-extra.gif"), lj_extra)
...
lj_g <- left_join(x, y_extra, by = "id") %>%
  proc_data() %>%
  mutate(.x = .x + 1) %>%
  plot_data_join("left_join(dat1,dat2,by='id')", ylims = ylim(-4.5, -0.5))
