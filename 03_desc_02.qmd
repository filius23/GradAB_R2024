...
Ein Blick in den Datenreport oder die Labels verrät mit `attributes()` uns aber die Ausprägungen:
```{r}
...
Kürzer ist für die PASS-Daten `< 0`, weil alle Missing-Codes kleiner 0 sind:[^match_op03]
```{r}
pend$statakt[pend$statakt < 0 ]  <- NA
```

[^match_op03]: Für nicht-systematische Werte wir den `%in%`-Operator verwenden, den wir schon im Zusammenhang mit [`filter()`](#filter) kennengelernt hatten: `pend$var1[pend$var1 %in% c(-9,2,124) ]  <- NA`  (das ist aber hier nur )
